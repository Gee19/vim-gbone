...
if exists('g:gbone_run_ft_map') && exists('g:gbone_run_mapping')
  ...
  for [ft, cmd] in items(g:gbone_run_ft_map)
    ...
    execute 'autocmd FileType ' . ft . ' nmap <buffer> ' . g:gbone_run_mapping . ' :call gbone#send_to_pane(''last'', ' . s:quote_char . cmd . s:quote_char . ', 1, ' . s:quote_char . s:ft_strategy . s:quote_char . ')<CR>'

    for [key, val] in items({'h':'left', 'j':'bottom', 'k':'top', 'l':'right'})
      execute 'autocmd FileType ' . ft . ' nmap <buffer> ' . g:gbone_run_mapping . key . ' :call gbone#send_to_pane(' . s:quote_char . val . s:quote_char . ', ' . s:quote_char . cmd . s:quote_char . ', 1, ' . s:quote_char . s:ft_strategy . s:quote_char . ')<CR>'
    endfor
